public interface ElementalAttacks {
    ...
    default void performSingleElementalAttack(int attackNumber, Monster opponent) {
        ...
        System.out.println(thisMonster.getName() + " used "
                + (attackNumber == 1 ? thisMonster.attackNameOne : thisMonster.attackNameTwo)
                + "!");
        double mult = 1.0;
        if (isSuperEffectiveAgainst(
                attackNumber == 1 ? thisMonster.attackElementOne : thisMonster.attackElementTwo,
                opponent.getElement())) {
            System.out.println("It was super effective!");
            mult = 2.0;
        } else if (isSuperEffectiveAgainst(opponent.getElement(),
                attackNumber == 1 ? thisMonster.attackElementOne : thisMonster.attackElementTwo)) {
            System.out.println("It was not very effective...");
            mult = 0.5;
        }
        boolean isCrit = didAttackCrit(thisMonster.speed);
        if (isCrit) {
            System.out.println("It was a critical hit!");
        }
        double totalDamage = (double) thisMonster.getAttack() / opponent.getDefense()
                * (thisMonster.getElement() == (attackNumber == 1 ? thisMonster.attackElementOne
                        : thisMonster.attackElementTwo) ? 1.5 : 1.0)
                * mult * (isCrit ? 2.0 : 1.0);
    ...
    default void performDoubleDownAttack(Monster opponent) {
        ...
        System.out.println(thisMonster.getName() + " used their double down attack!");
        performSingleElementalAttack(1, opponent);
        performSingleElementalAttack(2, opponent);
